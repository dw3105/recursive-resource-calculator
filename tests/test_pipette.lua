...
--T0/T1: Factorio's own pipette empties a hand holding the ghost of what the button shows; a Q paste survives that once and gives the ghost back

--gear holds an uncommon assembler with two modules and is copied; cog holds a normal assembler (the same machine), X a fast assembler
local function copied_onto_same_machine(world)
    chosen().gear = {name = "assembler", quality = "uncommon"}
    setups().gear = {modules = {{name = "speed-module"}, {name = "efficiency-module"}}, beacons = {}}
    chosen().cog = {name = "assembler"}
    setups().cog = {modules = {}, beacons = {}}
    chosen().X = {name = "fast-assembler"}
    local sheet_flow = sheet({{item = "gear"}, {item = "cog"}, {item = "X"}})
    press(world, machine_button(sheet_flow, "gear"))
    H.deep_equal(hand(), {name = "assembler", quality = "uncommon", ghost = true}, "T0a: a Q copy keeps the ghost it wrote")
    world.flush_cursor_events()
    world.advance_tick()
    drain(world)
    assert(clipboard(), "T0a: a Q copy keeps its clipboard")
    return sheet_flow
end

local function cog_state()
    return M.QualityLoops._deep_copy({machine = chosen().cog, setup = setups().cog})
end

for _, shape in ipairs(H.shapes()) do
    H.test(shape .. " T0a T1a Q with the copied machine's ghost over the same machine: vanilla empties the hand, the paste lands, the ghost comes back", function()
        local world = pipette_world(shape)
        local sheet_flow = copied_onto_same_machine(world)
        press(world, machine_button(sheet_flow, "cog"))
        H.equal(hand(), nil, "T0a: vanilla Q emptied the hand")
        world.flush_cursor_events()
        assert(clipboard(), "T1a: that one emptying is forgiven")
        world.advance_tick()
        drain(world)
        H.deep_equal(chosen().cog, {name = "assembler", quality = "uncommon"}, "T1a: machine pasted")
        H.equal(stored(setups().cog.modules), "speed-module,efficiency-module", "T1a: modules pasted")
        H.deep_equal(hand(), {name = "assembler", quality = "uncommon", ghost = true}, "T1a: ghost back in the hand")
        world.flush_cursor_events()
        assert(clipboard(), "T1a: the give-back notification is forgiven")
        world.advance_tick()
        drain(world)
        press(world, machine_button(sheet_flow, "X"))
        H.deep_equal(hand(), {name = "assembler", quality = "uncommon", ghost = true}, "T0a: over another machine the hand is kept")
        world.flush_cursor_events()
        world.advance_tick()
        drain(world)
        H.deep_equal(chosen().X, {name = "assembler", quality = "uncommon"}, "T1a: a third row pastes too")
    end)

    H.test(shape .. " T0a T1b Q with the copied group's ghost over a group of the same beacon: the group is replaced and the ghost comes back", function()
        local world = pipette_world(shape)
        local sheet_flow = copied_beacon_group(world)
        H.deep_equal(hand(), {name = "beacon", quality = "uncommon", ghost = true}, "T0a: a beacon copy keeps its ghost")
        assert(clipboard(), "T0a: and its clipboard")
        press(world, beacon_buttons(sheet_flow, "cog")[1])
        H.equal(hand(), nil, "T0a: vanilla Q emptied the hand")
        world.flush_cursor_events()
        world.advance_tick()
        drain(world)
        H.deep_equal(setups().cog.beacons, {clipboard().group}, "T1b: group replaced")
        H.deep_equal(hand(), {name = "beacon", quality = "uncommon", ghost = true}, "T1b: ghost back")
    end)

    H.test(shape .. " T1c T1d a real item picked up in the pressing tick, or a later clear and reselect, still drops the clipboard and pastes nothing", function()
        local world = pipette_world(shape)
        local sheet_flow = copied_onto_same_machine(world)
        local before = cog_state()
        press(world, machine_button(sheet_flow, "cog"))
        world.hold_item(1, "raw")
        world.flush_cursor_events()
        world.advance_tick()
        drain(world)
        H.equal(clipboard(), nil, "T1c: dropped")
        H.deep_equal(cog_state(), before, "T1c: nothing pasted")

        world = pipette_world(shape)
        sheet_flow = copied_onto_same_machine(world)
        before = cog_state()
        press(world, machine_button(sheet_flow, "cog"))
        world.flush_cursor_events()
        world.advance_tick()
        world.hold_ghost(1, "assembler", "uncommon")
        world.flush_cursor_events()
        drain(world)
        H.equal(clipboard(), nil, "T1d: a later notification drops it")
        H.deep_equal(cog_state(), before, "T1d: nothing pasted")
    end)

    H.test(shape .. " T1e a click with the ghost on the same machine pastes and expects no vanilla emptying", function()
        local world = pipette_world(shape)
        local sheet_flow = copied_onto_same_machine(world)
        left_click(machine_button(sheet_flow, "cog"), world.tick)
        H.equal(clipboard().vanilla_clear_tick, nil, "T1e: no emptying expected")
        H.equal(storage[1].pipette_requests[1].expect_vanilla_clear, nil, "T1e: request not flagged")
        world.advance_tick()
        drain(world)
        H.deep_equal(chosen().cog, {name = "assembler", quality = "uncommon"}, "T1e: pasted")
    end)

    H.test(shape .. " T1f an unflagged request (another machine) finds an empty hand at the drain and pastes nothing", function()
        local world = pipette_world(shape)
        local sheet_flow = copied_onto_same_machine(world)
        local before = M.QualityLoops._deep_copy({machine = chosen().X, setup = setups().X})
        press(world, machine_button(sheet_flow, "X"))
        world.suppress_next_cursor_event(1)
        world.empty_hand(1)
        world.advance_tick()
        drain(world)
        H.deep_equal(M.QualityLoops._deep_copy({machine = chosen().X, setup = setups().X}), before, "T1f: nothing pasted")
    end)

    H.test(shape .. " T1g (defensive no-clear profile) an unused allowance does not forgive an empty hand at a later tick", function()
        local world = pipette_world(shape)
        world.vanilla_pipette_clears = false
        local sheet_flow = copied_onto_same_machine(world)
        local before = cog_state()
        press(world, machine_button(sheet_flow, "cog"))
        world.flush_cursor_events()
        ...
        world.empty_hand(1)
        world.flush_cursor_events()
        H.equal(clipboard(), nil, "T1g: dropped")
        drain(world)
        H.deep_equal(cog_state(), before, "T1g: nothing pasted")
    end)

    H.test(shape .. " T1h (merged notifications) the pressing tick ending with a real item in hand drops the clipboard at once", function()
        local world = pipette_world(shape)
        local sheet_flow = copied_onto_same_machine(world)
        world.merge_cursor_events = true
        local before = cog_state()
        ...
        world.hold_item(1, "raw")
        world.flush_cursor_events()
        H.equal(clipboard(), nil, "T1h: dropped at the flush")
        world.advance_tick()
        drain(world)
        H.deep_equal(cog_state(), before, "T1h: nothing pasted")
    end)
end
